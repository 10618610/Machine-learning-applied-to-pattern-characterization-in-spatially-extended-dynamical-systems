...
integer::i,N,t,trans,seed,k,l,mmax,m,trans2,t2
integer:: passos,ai,epsi,passosa,passose
parameter(N=100)
parameter(Np=N)
real::u1(N),u2(N),soma(N),x(N),a,lambda(N),wi(np),wr(np),w(n),y(n),v(n),q1,q2,KS,h(N),difks
real:: S(N,N),D(N,N),Z(N,N),ksi,ksf
real:: amax,amin,epsmax,epsmin,apasso,epspasso,tempi,tempf
CHARACTER(len=15) :: FN
external f,ran0,der,balanc,elmhes,hqr, GRAMSCHMIDT, tred2
open(1,file='delta.dat')
...
passose=500
eps=0.1d0
apasso=(amax-amin)/float(passosa-1)
  !epspasso=(epsmax-epsmin)/dfloat(passose-1)

  do 1000 ai=0,passosa-1
  a=amin+apasso*float(ai)
   !do 1100 epsi=0,passose-1
